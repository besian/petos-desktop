...
const nwTimeOptions = ['08:30', '09:00', '11:00', '13:30', '15:00'];
...
export function NewWalkModal() {
  ...
  const pets = db.pets;
  const nwPetId = state.nwPetId || pets[0]?.id || null;
  const pet = pets.find((p) => p.id === nwPetId);
  const dayIso = weekDays[state.nwDay] || weekDays[mondayIndex(todayISO())];
  const rate = { 30: db.settings.rateSolo30, 45: Math.round((db.settings.rateSolo30 + db.settings.rateSolo60) / 2), 60: db.settings.rateSolo60 }[state.nwDur];
  const nwClash = db.walks.some((w) => w.date === dayIso && w.time === state.nwTime && w.status !== 'cancelled');

  const nwSummary = pet ? `${pet.name} · ${dow(dayIso)} ${state.nwTime} · ${state.nwDur} min` : '';
  const nwPrice = `£${rate}` + (state.nwRepeat ? ' /wk' : '');

  const handleAdd = () => {
    if (!pet) return;
    addWalk({ petId: pet.id, walkerId: db.team[0]?.id || '', date: dayIso, time: state.nwTime, durationMin: state.nwDur, price: rate, route: `${pet.name}'s usual route`, repeatWeekly: state.nwRepeat });
    actions.closeNewWalk();
    actions.showToast(`${pet.name} added${state.nwRepeat ? ' · weekly' : ''}`);
  };
      ...
      <div style={st('width:520px;max-width:calc(100% - 48px);background:var(--bg-app);border-radius:20px;border:1px solid var(--border-default);box-shadow:0 30px 80px -30px rgba(0,0,0,.5);overflow:hidden')}>
        <div style={st('padding:20px 24px 16px;display:flex;align-items:center;gap:12px;border-bottom:1px solid var(--border-subtle)')}>
          <div style={st('flex:1')}>
            ...
            <div style={st('font-size:12.5px;color:var(--fg-tertiary)')}>Book a walk for this week</div>
          </div>
        ...
        {pets.length === 0 ? (
          <div style={st('padding:30px 24px;text-align:center;font-size:13.5px;color:var(--fg-tertiary)')}>Add a pet first before scheduling a walk.</div>
        ) : (
          <>
            <div style={st('padding:20px 24px')}>
              ...
              <div className="ps" style={st('display:flex;gap:9px;margin-bottom:20px;overflow-x:auto')}>
                {pets.map((p) => {
                  const on = nwPetId === p.id;
                  return (
                    <button
                      key={p.id}
                      onClick={() => actions.setNwPet(p.id)}
                      style={st(`${nwSeg};border:1px solid ${on ? 'var(--border-brand)' : 'var(--border-subtle)'};background:${on ? 'var(--bg-brand-subtle)' : 'var(--bg-primary)'};flex:none;width:70px`)}
                    >
              ...
              >
                ...
                <div style={st('flex:1')}>
                  ...
                  <div style={st('font-size:12px;color:var(--fg-tertiary)')}>{state.nwRepeat ? 'Every week at this time · skip anytime' : 'One-off walk'}</div>
                </div>
                <span style={st(`width:40px;height:23px;border-radius:99px;flex:none;display:inline-flex;align-items:center;padding:2px;background:${state.nwRepeat ? 'var(--brand-primary)' : 'var(--bg-tertiary)'}`)}>
                  <span style={st(`width:19px;height:19px;border-radius:99px;background:#fff;${state.nwRepeat ? 'margin-left:auto' : ''}`)} />
                </span>
              </button>
              {nwClash ? (
            ...
            <div style={st('padding:14px 24px;border-top:1px solid var(--border-subtle);display:flex;align-items:center;gap:14px')}>
              ...
              <button onClick={handleAdd} style={st('border:none;background:var(--brand-primary);color:var(--brand-on-primary);font-family:inherit;font-size:14.5px;font-weight:600;padding:11px 22px;border-radius:11px;cursor:pointer;box-shadow:var(--shadow-ring-primary)')}>Add walk</button>
            </div>
          </>
        )}
      </div>
