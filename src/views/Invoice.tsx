...
export function Invoice() {
  ...
  const statusMap: Record<string, { label: string; pill: string; foot: string; glyph: string; ftext: string; primary: string; primaryAction: () => void; secondary: string; secondaryAction?: () => void }> = {
    paid: { label: 'Paid', pill: bigPill('background:var(--color-success-50);color:var(--color-success-700)'), foot: 'background:var(--color-success-50);color:var(--color-success-700)', glyph: '✓', ftext: `Paid ${inv.paidOn || ''} · ${inv.method || ''}`, primary: 'Send receipt', primaryAction: () => actions.showToast('Receipt sent'), secondary: '' },
    outstanding: { label: 'Due', pill: bigPill('background:var(--color-warning-50);color:var(--color-warning-700)'), foot: 'background:var(--bg-secondary);color:var(--fg-secondary)', glyph: '○', ftext: `Payment due ${inv.due}`, primary: 'Send reminder', primaryAction: () => actions.showToast('Reminder sent to ' + client.name), secondary: 'Mark as paid', secondaryAction: () => { markInvoicePaid(inv.id); actions.showToast('Marked as paid'); } },
    overdue: { label: 'Overdue', pill: bigPill('background:var(--color-error-50);color:var(--color-error-700)'), foot: 'background:var(--color-error-50);color:var(--color-error-700)', glyph: '!', ftext: `Overdue since ${inv.due}${inv.reminderOn ? ' · last reminder ' + inv.reminderOn : ''}`, primary: 'Send reminder', primaryAction: () => actions.showToast('Reminder sent to ' + client.name), secondary: 'Mark as paid', secondaryAction: () => { markInvoicePaid(inv.id); actions.showToast('Marked as paid'); } },
    draft: { label: 'Draft', pill: bigPill('background:var(--bg-tertiary);color:var(--fg-tertiary)'), foot: 'background:var(--bg-secondary);color:var(--fg-tertiary)', glyph: '○', ftext: `Draft — not yet sent to ${client.name}`, primary: 'Send invoice', primaryAction: () => { sendInvoice(inv.id); actions.showToast('Invoice sent to ' + client.name); }, secondary: '' },
  };
  ...
  return (
    <div style={st('animation:vIn .3s var(--ease-out);max-width:840px;margin:0 auto')}>
      <div style={st('display:flex;align-items:center;gap:10px;margin-bottom:18px')}>
        ...
        {status.secondary ? <button onClick={status.secondaryAction} style={st(outlineBtn)}>{status.secondary}</button> : null}
        <button onClick={status.primaryAction} style={st('border:none;background:var(--brand-primary);color:var(--brand-on-primary);font-family:inherit;font-size:13px;font-weight:600;padding:9px 16px;border-radius:10px;cursor:pointer;box-shadow:var(--shadow-ring-primary)')}>{status.primary}</button>
      </div>
