...
export function TeamChat() {
  ...
  usePageHeader('Chat', member?.name || 'Not found');
  ...
  const sendInvite = async () => {
    ...
    const link = `${window.location.origin}/portal/signup?email=${encodeURIComponent(member.email)}`;
    ...
    setInviting(false);
    actions.showToast(result.ok ? `Invite sent to ${member.name}` : result.error || 'Could not send invite');
  };
  ...
  return (
    <div style={st('animation:vIn .3s var(--ease-out);max-width:640px;margin:0 auto;height:calc(100vh - 140px);display:flex;flex-direction:column')}>
      <button onClick={() => navigate(`/team/${member.id}`)} style={st('display:inline-flex;align-items:center;gap:7px;border:1px solid var(--border-default);background:var(--bg-primary);color:var(--fg-secondary);font-family:inherit;font-size:13px;font-weight:600;padding:8px 13px;border-radius:10px;cursor:pointer;margin-bottom:14px;flex:none;align-self:flex-start')}>
        <ChevronLeftIcon />{member.name}
      </button>
      {!member.authUserId ? (
        <div style={st('display:flex;align-items:center;gap:12px;background:var(--bg-brand-subtle);border:1px solid var(--border-brand);border-radius:12px;padding:11px 14px;margin-bottom:12px;flex:none')}>
          <div style={st('flex:1;font-size:12.5px;color:var(--fg-brand)')}>{member.name} hasn't joined chat yet. Messages you send now will be waiting for them once they do.</div>
          <button
            onClick={sendInvite}
            disabled={inviting}
      ...
      <div style={st('flex:1;min-height:0;background:var(--bg-primary);border:1px solid var(--border-subtle);border-radius:18px;box-shadow:var(--card-shadow);overflow:hidden')}>
        <ChatThread ownerId={account.id} threadType="team" threadId={member.id} myRole="owner" counterpartName={member.name} />
      </div>
    </div>
  );
}
