...
export function PortalSignup() {
  ...
  async function onSubmit(e: FormEvent) {
    e.preventDefault();
    setSubmitting(true);
    const res = await portalSignup({ email, password });
    setSubmitting(false);
    if (!res.ok) { setError(res.error || 'Could not set up chat access.'); return; }
    ...
    <div style={st('min-height:100vh;width:100%;display:flex;align-items:center;justify-content:center;background:var(--bg-secondary);padding:20px;box-sizing:border-box')}>
      <div style={st('width:100%;max-width:400px')}>
        ...
        <form onSubmit={onSubmit} style={st('background:var(--bg-primary);border:1px solid var(--border-subtle);border-radius:18px;box-shadow:var(--card-shadow);padding:28px')}>
          <div style={st('font-size:19px;font-weight:700;color:var(--fg-primary);margin-bottom:4px')}>Set up chat access</div>
          <div style={st('font-size:13px;color:var(--fg-tertiary);margin-bottom:22px')}>Use the same email your dog walker has on file for you — we'll match it up automatically.</div>
          ...
          <div style={st('text-align:center;font-size:13px;color:var(--fg-tertiary);margin-top:18px')}>
            Already set up? <Link to="/portal/login" style={st('color:var(--fg-brand);font-weight:600')}>Log in</Link>
          </div>
        </form>
        <div style={st('text-align:center;font-size:11.5px;color:var(--fg-quaternary);margin-top:16px;line-height:16px')}>
          Are you the business owner? <Link to="/signup" style={st('color:var(--fg-tertiary);text-decoration:underline')}>Set up your business here instead</Link>
        </div>
      </div>
    </div>
  );
